			  ...
			  const MTK_WCN_HIF_SDIO_FUNCINFO *prFuncInfo)
{
	...
	case WMTHWVER_MT6620_E5:
		firmware_download("/system/etc/firmware/WIFI_RAM_CODE", cltCtx);
		break;
	case WMTHWVER_MT6620_E6:
	default:
		firmware_download("/system/etc/firmware/WIFI_RAM_CODE_E6", cltCtx);
		break;
	}
#elif defined(MT5931)
	firmware_download("/system/etc/firmware/WIFI_RAM_CODE", cltCtx);
#elif defined(MT6628)
	firmware_download("/system/etc/firmware/WIFI_RAM_CODE_MT6628", cltCtx);
#endif
